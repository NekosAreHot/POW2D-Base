...
Game.checkChest = function(num){
	if(Game.getChest(Game.player.x,Game.player.y) != null && Game.getChest(Game.player.x,Game.player.y).getContents(num ) != null){
		return Game.getChest(Game.player.x,Game.player.y).getContents(num).name;
	}else{
		return "Empty";
	}
}
Game.getChestItem = function(num){
	if(Game.getChest(Game.player.x,Game.player.y) != null && Game.getChest(Game.player.x,Game.player.y).getContents(num ) != null){
		return Game.getChest(Game.player.x,Game.player.y).getContents(num);
	}else{
		return "empty.png";
	}
}
Game.createLockedChest = function(x,y,contents){
	Game.chests.push(new Game.GameObject(20,20,Game.File.getImageDirectory() + "chest_texture.png",x,y,"lockedchest"));
	var obj = Game.chests[Game.chests.length - 1];
	obj.lock();
	for(var i = 0; i < contents.length; i++){
		Game.chests[Game.chests.length - 1].setContents(contents[i]);
...
Game.unlockChest = function(item){
	for(var i = 0; i < Game.chests.length; i++){
		if(Game.chests[i].type == "lockedchest"){
			Game.chests[i].unlock(item);
		}
	}
}