...
*/
Game.File.Create = function(directory, data,con){
fs.writeFile(directory,data, (err) => {
	if (err) throw err;
	if (con) console.log("Made " + data + " in " + directory);
});
};
/**
...
*/
Game.File.CreateDir = function(directory,con){
	fs.mkdir(directory, 511, function(err) {
			if (err) throw err;
			if (con) console.log("made " + directory);
	});
};
/**
...
if(typeof(Storage) !== "undefined"){
	...
	*/
	...
	Game.Console.sendError("No WebStorage Support");
}
if(typeof(Worker) !== "undefined"){
	...
	Game.Console.sendError("No WebWorker Support");
}
